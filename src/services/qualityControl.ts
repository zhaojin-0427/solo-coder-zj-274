...
import { calculateOrderLayout, calculateLayout } from '../utils/layout'
...
export function emptyDefectCounts(): Record<QCDefectType, number> {
...
export interface ExtendedPlacement extends PlacedPattern {
  orderId?: string | null
...
export function createPageCheck(pageIndex: number): QCPageCheck {
  return {
    pageIndex,
    status: 'pending',
    defects: [],
...
export interface CreateQCInspectionSessionParams {
  batchId: string | null
  batchName: string
...
export function createQCInspectionSession(params: CreateQCInspectionSessionParams): QCInspectionSession {
  ...
  for (let i = 0; i < params.placements.length; i++) {
    const pl = params.placements[i] as ExtendedPlacement
...
export function computeQCBatchStats(
  ...
  const totalPages = placements.length > 0 ? Math.max(0, ...placements.map(p => p.pageIndex)) + 1 : 0
...
export interface ReworkItemAggregated {
  patternId: string
  orderId: string | null
  orderItemId: string | null
  quantity: number
...
export interface GenerateReworkBatchParams {
  session: QCInspectionSession
  placements: (PlacedPattern | PlacedPatternWithOrder)[]
...
export function generateReworkBatch(params: GenerateReworkBatchParams): ReworkBatch {
  const { session, placements, patterns, settings, calibration, orders } = params

  const reworkMap = new Map<string, ReworkItemAggregated>()
  ...
  for (const pageCheck of Object.values(session.pageChecks)) {
    for (const pc of Object.values(pageCheck.patternChecks)) {
      ...
      if (!pl) continue
      const extPl = pl as ExtendedPlacement

      const key = `${pl.patternId}-${extPl.orderId || 'none'}-${extPl.orderItemId || 'none'}-${pl.nailSize}-${pl.nailShape}`
      if (!reworkMap.has(key)) {
        reworkMap.set(key, {
          patternId: pl.patternId,
          orderId: extPl.orderId || null,
          orderItemId: extPl.orderItemId || null,
          quantity: 0,
  ...
  let reworkPageInfo: PageLayoutInfo[] = []
  ...
  if (reworkItems.length > 0) {
    if (session.sourceType === 'order' && orders && orders.length > 0) {
      ...
      if (reworkPatterns.length > 0) {
        ...
        reworkPlacements = result.placements.map((p): PlacedPatternWithOrder => ({
          ...p,
          orderId: null,
          orderNo: null,
          orderColorTag: null,
